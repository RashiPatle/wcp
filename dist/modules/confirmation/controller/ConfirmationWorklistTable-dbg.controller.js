sap.ui.define([
	...
	return BaseController.extend("com.westernacher.collaborationportal.confirmation.controller.ConfirmationWorklistTable", {
		...
		onInit: function () {
			...
			// this.oComponent._ConfH.setRoleModel(true, this.getView(), "CONFIRMATION", this.oComponent.defaultAnnotation);
			// this.getView().getModel().getMetaModel().loaded().then(function (oEvent) {
			// 	//Do we need that if?
			// 	if (this.oComponent.ownerComponent) {
			// 		try {
			// 			this.oComponent._AH.checkUserRoleMetaModel(this.getView().getModel(), this.oComponent._ConfH.sUserRole);
			// 		} catch (er) {
			// 			return;
			// 		}
			// 	}
			// }.bind(this));
			var oSmartTable = this.getView().byId("fosConfirmationTable");
		...
		onTableItemPress: function (oEvent) {
			...
			this.oComponent.getRouter().navTo("object", {
				UserRole: this.oComponent._ConfH.sUserRole,
		...
		onUpdateFOPress: function () {
			this.oComponent._ConfH.onUpdateFreightOrderPress(this.onUpdateFODatesWorkListSuccess.bind(this), this.onUpdateFODatesWorkListError
				.bind(this));
		},
		...
		onRefreshList: function () {
			this.onRefreshTableList(this.getView().byId("fosConfirmationTable"));
		}
