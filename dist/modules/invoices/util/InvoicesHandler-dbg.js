sap.ui.define([
	...
	return ParentModuleHandler.extend("com.westernacher.collaborationportal.invoices.util.InvoicesHandler", {
		constructor: function (oComponent) {
			...
			this._oCIModel = this._oComponent.getModel("_CI");
			...
			this.sInvoicesEntitySet = "ZWCP_C_SETLCarrierInvoices";
			this.i18n = oComponent.getModel("i18n");
			this.updateFOQuantitiesDialogId = "idUpdateFOQuantitiesInvoicecDialog";
			this.addChargeDialogId = "idAddChargeInvoicecDialog";
			this.updateChargeDialogId = "idUpdateChargeInvoicecDialog";
		},

		/*Configuration object - charge types - fetched once
		 */
		...
		ValidatingAmountsInLocale: function (oEvent) {
			...
			formatter.validatePriceInLocaleFormat(oEvent);
		},
		...
		getTaxItemsOut: function (oFreightOrder, aChargeItems) {
			/*	var aTaxItems = this._oCIModel.getProperty("/FreightOrders/TaxItems");
				...
				this._oCIModel.setProperty("/FreightOrders/TaxItems", aTaxItems);*/
			/*	var aFO = this._oCIModel.getProperty("/FreightOrders");
				var aTaxItems = aFO.to_TaxItems;*/
			var aTaxItems = [];
			for (var i = 0; i < aChargeItems.length; i++) {
				if (aChargeItems[i].TaxIndicator) {
					aTaxItems.push(aChargeItems[i]);
		...
		onSubmitInvoice: function (oEvent) {
		...
		_navigateToList: function () {
			this._oComponent.getRouter().navTo("master", {});
		},
		...
		_prepareUpdateObject: function () {
			...
			if (oTotalDistanceInput && oTotalDistanceInput.getEnabled()) {
				oFO.ChangedTotalDistance = oTotalDistanceInput.getValue();
				anyAvailable = true;
			}
			...
			if (oChangedGrossVolumeInput && oChangedGrossVolumeInput.getEnabled()) {
				oFO.ChangedGrossVolumeValue = oChangedGrossVolumeInput.getValue();
				anyAvailable = true;
			}
			if (oChangedGrossWeightInput && oChangedGrossWeightInput.getEnabled()) {
				oFO.ChangedGrossWeight = oChangedGrossWeightInput.getValue();
				anyAvailable = true;
			}
			if (anyAvailable) {
				return oFO;
		...
		**/
		...
		onUpdateChargeConfirmed: function () {
			...
			//checking for any changes done on confirming
		...
		callChargeItemsData: function (oFO, sPathFO, sTransportationOrderUUID, sInvoiceKey, fnCallBack) {
			...
			var sFOKey = this.getModel().createKey("ZWCP_C_SETLFreightOrders", {
			...
			this.getModel().read("/" + sFOKey, {
				...
				success: function (oData, oResponse) {
					...
					/*oFreightOrder.ChangedTotalDistance = formatter.numberUnit(oFreightOrder.ChangedTotalDistance);
					oFreightOrder.ChangedGrossVolumeValue = formatter.numberUnit(oFreightOrder.ChangedGrossVolumeValue);
					oFreightOrder.ChangedGrossWeight = formatter.numberUnit(oFreightOrder.ChangedGrossWeight);
					oFreightOrder.TotalDistance = formatter.numberUnit(oFreightOrder.TotalDistance);
					oFreightOrder.TranspOrdGrossVolume = formatter.numberUnit(oFreightOrder.TranspOrdGrossVolume);
					oFreightOrder.TranspOrdGrossWeight = formatter.numberUnit(oFreightOrder.TranspOrdGrossWeight);*/
