sap.ui.define([
	...
	return BaseController.extend("com.westernacher.collaborationPortal.freightUnit.controller.freightUnitObjectPage", {
		...
		onPressBack: function () {
			...
			if (sPreviousHash !== undefined) {
				...
				oHashChanger.init();
				oHashChanger.setHash("FU/" + _oMDModel.getProperty("/UserInfo/CurrentBusinessPartnerRole"));
			}
		},
		...
		onRouteMatched: function (oEvent) {
			...
			var oModel = this.oComponent._FreightUnitH.setRoleModel(true, this.getView(), "FU", this.oComponent.defaultAnnotation);
			oModel.getMetaModel().loaded().then(function () {
				try {
					this.oComponent._AH.checkUserRoleMetaModel(oModel, this.oComponent._FreightUnitH.sUserRole);
					this.oComponent._AH.checkUserRoleMetaModelForEntity(oModel, this.oComponent._FreightUnitH.sUserRole, "FreightUnit");
				} catch (er) {
					return;
				}
			}.bind(this));
			...
			oModel.metadataLoaded().then(function () {
				this.sObjectPath = oView.getModel().createKey("FreightUnits", {
		...
		onUpdateFUPress: function () {
			...
			var sFU = "";
			var aFU = this.oComponent.getModel("_SD").getProperty("/SelectedItems")[0];

			//condition here to check 24 hours validation
			// var oLoadReadyDate = aFU.getProperty("LoadReadyDate");
			// var oNow = new Date();
			// var diffInSeconds = (oLoadReadyDate.getTime() - oNow.getTime()) / 1000;

			// if (diffInSeconds / 86400 < 1) {
			// 	bValidation = false;
			// 	sFU = aFU.getProperty("FreightUnitID") + " ";
			// }

			//Dummy logic for testing UI
			if (aFU.getProperty("FreightUnitID") === "4100000100" || aFU.getProperty("FreightUnitID") === "4100000101") {
				bValidation = false;
				sFU = aFU.getProperty("FreightUnitID") + " ";
			}

			if (bValidation) {
				this.oComponent._FreightUnitH.onUpdateFreightUnitPress(this.onUpdateFUObjectPageSuccess.bind(this), this.onUpdateFUObjectPageError
					.bind(this));
			} else {
